...
class PivotBlockingQueue:
    def __init__(self,dim,pivotNumber=1):
        self.dim = dim
        self.buffer = []
        self.criterio = True
        self.lock = RLock()
        self.condNewElement = Condition(self.lock)
        # il pivotNumber deve essere da 1 a N-1
        self.pivotNumber = pivotNumber if pivotNumber < dim else 1 
    ...
    def waitFor(self, n):
        with self.lock:
            ...
            print ("Controllo se n è maggiore della somma dei valori presenti nella coda") 
            while n > self.__somma__(): 
                self.condNewElement.wait()
                print ("la somma attuale è ",self.__somma__())
            print (f"La somma dei valori presenti nella coda Ã¨ maggiore di {n}") 
            
    def __somma__(self):
        return sum(self.buffer) if len(self.buffer) > 0 else 0  
...
class Tester(Thread):
    ...
    def run(self):
        for i in range(10):
            self.coda.waitFor(10000)
            print("Stampo : "+self.coda.doubleTake()) 
        
            

if __name__ == '__main__':
            
    coda = PivotBlockingQueue(10, 2)        
