...
class BanconeBar:
    def __init__(self, righe, colonne):
        ...
        self.colonnaPrioritaria = None 
    
    def imposta_colonna_prioritaria(self, i): 
        with self.lock:
            if 0 <= i < self.colonne:
                self.colonnaPrioritaria = i
            else:
                self.colonnaPrioritaria = None 
    ...
    def get(self):
        with self.lock:
            ...
            if self.colonnaPrioritaria in indiciDaCuiScegliere:
                datiElemento = self.bancone[self.colonnaPrioritaria].pop(0) 
            else:
