...
class BlockingQueue:
    ...
    def get(self):
        with self.lock:
            ...
            self.create_a_log("get",self.elementi[0]) 
            return self.elementi.pop(0)
        
    def create_a_log(self, operazione, elemento):
        with self.lock:
            if self.logging:
                self.logger.apppend((operazione,elemento, time.time()))
                if len(self.logger) > self.M: 
                    self.logger.pop(0)
    ...
    def read_get_log(self,o):
        with self.lock:
            if self.logging == False:
                print ("Non sto facendo logging")
                return 
            # devo returnare l'ultima volta che o è stato preso dalla lista logger
            for i in range(len(self.logger)-1,-1,-1):
                if self.logger[i][0] == "get" and self.logger[i][1] == o:
                    return self.logger[i][2]
    ...
    def read_put_log(self,o):
        with self.lock:
            if self.logging == False:
                print ("Non sto facendo logging")
                return 
            # devo returnare l'ultima volta che o è stato messo nella lista logger
            for i in range(len(self.logger)-1,-1,-1):
                if self.logger[i][0] == "put" and self.logger[i][1] == o:
                    return self.logger[i][2] 
            return -1  
    ...
    def read_diff_log(self,o):
        with self.lock:
            if self.logging == False:
                print ("Non sto facendo logging")
                return 
            # devo returnare la differenza tra l'ultima volta che o è stato messo e l'ultima volta che è stato preso
            get_time = self.read_get_log(o)
            put_time = self.read_put_log(o)

            if get_time == -1 or put_time == -1:
                return -1 
            return get_time - put_time 
