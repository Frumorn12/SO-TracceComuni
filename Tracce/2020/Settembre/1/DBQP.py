    ...
    def takeLast(self):
        with self.wlock:
            ...
            if len(self.scadenze) > 0:
                # prendo la chiave con il valore massimo di
                # scadenza tra tutti gli elementi in scadenza
                # e la restituisco come ultima palluzza

                # key = lambda x: self.scadenze[x] restituisce il valore della scadenza
                # per ogni elemento x della lista self.scadenze
                ultimaPalluzza = max(self.scadenze, key=lambda x: self.scadenze[x])

                del self.scadenze[ultimaPalluzza] # rimuovo l'elemento dalla lista degli elementi in scadenza
