...
class DatoCondiviso():
    ...
    def acquireReadLock(self):
        ...
        while self.numLockScrittura > 0 and self.currentWriter != current_thread():
            dprint(
                f"Il thread {current_thread().name} voleva leggere ma trova che c'̬ lo scrittore {self.currentWriter}. Dunque aspetta.")
            self.condition.wait()
        self.numLettori += 1
    ...
    def acquireWriteLock(self):
        ...
        while self.numLettori > 0 or self.numLockScrittura > 0 and self.currentWriter != current_thread():
            dprint(
                f"Il thread {current_thread().name} voleva scrivere su {self}, ma trova che ci sono {self.numLettori} lettori e che currentWrite={self.currentWriter}. Dunque aspetta.")
            self.condition.wait()
...
class InteriCombinati:
    def __init__(self):
        ...
        #
        self.operazioni = {'+': operator.add, '-': operator.sub, '*': operator.mul, '/': operator.truediv,
                           '%': operator.mod}
    ...
    def calcola(self, i, j, op):
        ...
        #
        for indice in sorted([i, j]): # Ordino gli indici per evitare deadlock
            self.V[indice].acquireReadLock() # Acquisisco il lock in lettura

        try:   # Utilizzo un blocco try finally per garantire che i lock vengano rilasciati anche in caso di eccezione
            if op in ['/', '%'] and self.V[j].getDato() == 0: # Controllo divisione per zero
                dprint(
                    f"Divisione per zero: non eseguo l'operazione {op} su V[{i}]={self.V[i].getDato()} e V[{j}]={self.V[j].getDato()}")
                raise ZeroDivisionError() # Lancio una eccezione per evitare di eseguire l'operazione
            else:
                if op == '+':
        ...
        finally: # Rilascio i lock in ordine inverso rispetto a quello in cui li ho acquisiti
            self.V[i].releaseReadLock()
            self.V[j].releaseReadLock()

    def aggiorna(self, i, j, k, op):

        indici_ordinati = sorted([i, j, k]) # Ordino gli indici per evitare deadlock
        #
        ...
        #
        for indice in indici_ordinati: # Acquisisco i lock in ordine crescente
            if indice == k: # Se l'indice ̬ k, acquisisco il lock in scrittura
                self.V[indice].acquireWriteLock() # Acquisisco il lock in scrittura
            else:
...
class Calcolatrice:
    def __init__(self): # Inizializzo la calcolatrice
        self.IC = InteriCombinati() # Inizializzo l'oggetto InteriCombinati
        self.B = Queue(10) # Inizializzo la coda B
        # Inizializzo i thread produttori e elaboratori
        # In particolare, 4 thread elaboratori e 2 thread produttori
        # Uso Thread per creare i thread in modo da poterli avviare in modo asincrono
        # target ̬ la funzione che il thread deve eseguire

        self.elaboratori = [Thread(target=self.elabora) for _ in range(4)] # Inizializzo 4 thread elaboratori
        self.produttori = [Thread(target=self.produce) for _ in range(2)] # Inizializzo 2 thread produttori
