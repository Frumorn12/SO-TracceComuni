...
#Potrei usare --help ma con man Ã¨ piÃ¹ semplice gestire le descrizioni: sono tutte su un'unica riga, non ci sono \n di mezzo
@doc = qx(man $comm);
...
for(@doc){
    if($op eq "-o" && $opzione ne ""){#Ho giÃ  incontrato un'opzione ma non ancora la sua descrizione
        ...
        if($op eq "-n"){
            ...
            if($4 =~ /^\s([^\n]+)/){#$4 puÃ² essere uno \n e quindi la descrizione Ã¨ sulla riga successiva, oppure qualcosa che non sia \n e quindi la descrizione Ã¨ sulla stessa riga
                $descr{$opzione}=$1;
                $opzione="";
            }
        }
